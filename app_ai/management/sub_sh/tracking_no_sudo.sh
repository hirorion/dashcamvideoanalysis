...
echo "docker run --rm -e CUR_USER_NAME=$1 -e USER_DIR=\"$2\" -e MOVIE_PREFIX=\"$3\" -e MOVIE_FILENAME=\"$4\" -e CAMERA_TYPE=\"$5\" -e CAMERA_HEIGHT=\"$6\" -e START_FNO=$7 -e END_FNO=$8 -v $9:/mnt/ --runtime=nvidia ${10}"
docker run --rm -e CUR_USER_NAME=$1 -e USER_DIR="$2" -e MOVIE_PREFIX="$3" -e MOVIE_FILENAME="$4" -e CAMERA_TYPE="$5" -e CAMERA_HEIGHT="$6" -e START_FNO=$7 -e END_FNO=$8 -v $9:/mnt/ --runtime=nvidia ${10}
